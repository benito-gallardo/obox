...
// 08.03.16 Katie Garcia
// Added {clickToOpen: true} as option on tooltips
// Default: false
...
(function ($) {
    $.fn.tooltip = function (options) {
      ...
      if (options.clickToOpen === true){
        origin.on( "click", function(e) {
          origin.trigger( "openTooltip" );
        });
        $(window).on('click', function(e){
          var container = $(".material-tooltip");
          if(container.is(":visible")) {
            origin.trigger( "closeTooltip" );
          }
          // if click is outside origin
          // if (!origin.is(e.target) && origin.has(e.target).length === 0) { ... }
        });
      } else {
        origin.on({
